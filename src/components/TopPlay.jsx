    ...
    <div ref={divRef} className='xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col'>
      <div className="w-full flex flex-col">
        ...
        <div className="mt-4 flex flex-col gap-1">
          {topPlays?.map((song, i)=>(
            <TopChartCard isPlaying={isPlaying} activeSong={activeSong} handlePauseClick={handlePauseClick} handlePlayClick={() => handlePlayClick(song,i)} song={song} i={i} key={song.key}/>
          ))}
        </div>
      </div>
      ...
      <div className="w-full flex flex-col mt-8">
        <div className="flex flex-row justify-between items-center">
          ...
          <Swiper slidesPerView="auto" spaceBetween={15} FreeMode centeredSlides centeredSlidesBounds modules={[FreeMode]} className="mt-4">
            {topPlays?.map((song, i)=>(
              <SwiperSlide key={song?.key} style={{width: "25%", height: 'auto'}} className="shadow-lg rounded-full animate-slider-right">
                <Link to={`/artists/${song?.artists[0].adamid}`}>
